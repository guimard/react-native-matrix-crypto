...
use matrix_crypto_core::{
    ...
    SenderTrustRequirement, SenderVerification,
    SessionError,
};
...
fn a_signing_identity_published_to_a_real_homeserver_and_what_a_sender_then_reads() {
    ...
    let from_nio = run(decrypt_event(&scope, &nio_raw_event.to_string(), SenderTrustRequirement::Any))
    ...
    let from_own = run(decrypt_event(&scope, &own_raw_event.to_string(), SenderTrustRequirement::Any))
