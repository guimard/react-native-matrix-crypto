...
fn three_devices_across_two_federating_homeservers() {
    ...
    let recovered = run(decrypt_event(&scope, &nio1_raw_event.to_string(), SenderTrustRequirement::Any))
    ...
    let recovered = run(decrypt_event(&scope, &nio2_raw_event.to_string(), SenderTrustRequirement::Any))
