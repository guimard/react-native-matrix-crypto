...
/// A `/keys/query` answer that names no identity for this account: the
/// server has been asked and has said there is none. Every field of ruma's
/// own response type is `#[serde(default)]`, so an empty object is a
/// well-formed answer meaning exactly that.
