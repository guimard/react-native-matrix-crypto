...
/// through states none of these name, so it is reported as the nearest one
/// and [`confirm_scan`] cannot tell a caller whether to wait or to start
/// again. Named at [`code_of`], where the mapping actually happens, and at
/// [`confirm_scan`]. Growing this enum is the task that closes it, and the
/// per-variant sentences below are written for the tree as it is rather
/// than for the tree it is becoming.
...
pub enum FlowStage {
    ...
    /// is nothing to show.
    ///
    /// **Also what a flow that became a code reports**, for as long as its
    /// request says `Transitioned`, which is the gap named on the enum
    /// above. On such a flow no comparison has started and there *is*
    /// something to show.
    Started,
    ...
    /// Not reached by a flow proceeding by a code, for the same reason.
    Confirmed,
...
fn stage_of(record: &mut FlowRecord) -> FlowStage {
    ...
    match request.state() {
        ...
        // **Reachable, and reached on every flow that became a code.**
        // This said it was unreachable, on the grounds that `comparison_of`
        // returns `Some` for exactly this state and would have returned
        // before the match. That held while a comparison was the only thing
        // a request could transition into. `comparison_of` reads
        // `verification.sas_v1()`, which is `None` for a transitioned
        // request holding a `QrVerification`, so a code flow arrives here
        // and is told it is `Started`. That is the wrong answer rather than
        // a truthful mapping of an unreachable arm, and it is named as a
        // defect at [`code_of`], which also says why teaching `stage_of` to
        // read the code handle before there is a stage for *scanned* to be
        // reported as would replace one wrong answer with another.
...
///
/// **A comparison, and only a comparison.** A flow finished by scanning a
/// code verifies a device and produces nothing here, because this reads
/// `SasState::Done` and a code flow never reaches one. So a product that
/// waits on [`crate::CryptoSignal::TrustChanged`] after a scan waits
/// forever, and `crate::device_statuses` is the answer that does move. That
/// is a gap rather than a decision, it belongs to the task that grows the
/// stage vocabulary, and it is named here rather than left in the shape of
/// a doc comment that reads as though it enumerated every completion this
/// library announces.
///
