...
**Every build gate has been watched rejecting a real violation**, not merely passing. There are eleven, they all run in CI, and a gate nobody has watched fail is not known to work. This sentence said ten while the table below listed eleven and `package.json` declared eleven. `gate:readme` held the rows, the scripts and the workflow to each other and counted nothing, so nothing failed; it counts now, and reads this sentence to do it.

**There is a working application to start from.** `packages/example-app` is a neutral React Native app that drives the encryption chain live on a device: it creates a machine, drains the outbound queue, encrypts and decrypts a real event, reads identity keys, and shows for each step the exact TypeScript a consumer would write next to what it got back. It said "the whole surface", and the surface has outgrown it: no card touches device verification, by either method, and `packages/example-app/README.md` says what else only a person holding a phone reaches. It imports from `react-native-matrix-crypto` and from nothing else, so what you read there is what you can write, with no private entry point doing the interesting part off screen. Copying from it is a reasonable way to start.
...
**There are two ways, and the second one arrived in this release.** Two people compare a seven-symbol string, or one of them points a camera at a code on the other's screen. Both end in the same place, both are described below, and a product may ship either or both. Everything up to the moment they diverge is one flow with one set of calls: this section is the string, and [verifying by a scannable code](#verifying-by-a-scannable-code) is the other. Until this release the string was the only method here, which is why so much of what follows is written in its vocabulary.
...
#### Ask for it, because nothing happens until you do

```ts
import { offerScannableCodes } from 'react-native-matrix-crypto'

offerScannableCodes(true)   // once at start-up, next to createCryptoMachine
```

**Off until you call this**, and both settings cost something, so both are written down here rather than left to be discovered.

**On when your product cannot really scan** tells the other side's client that this one can, so it shows its user a code and asks them to point a camera at it. Nothing here can read it, and **no error reaches either product**, because nothing was asked of this library. The flow stalls until the protocol's own ten-minute timeout, the person who pays is a user who did nothing wrong, and neither product can see it happen.

**Off when you meant to be on** refuses `getVerificationCode` with `code_not_offered` on the first flow you try it on: a named error, at integration time, in front of the person who can fix it in one line. That asymmetry is the whole argument for the default.
...
It applies to the whole process rather than to one flow, because it describes your product, and a product does not have a camera on some of its verifications and not others. What a flow announces is fixed when that flow is created or agreed to, so calling this afterwards changes nothing about a verification already under way. It is the one call on this surface that is not asynchronous: it sets a flag and cannot fail, and an awaitable call somebody forgot to await could land after the flow it was meant to affect had already said what it can do.
...
|---|---|---|
| `code_not_offered` | codes were not negotiated on this flow, and no amount of waiting changes it | two causes, and you can always tell which, because one of them is your own state: either this build never called `offerScannableCodes`, or the other device did not offer to scan. Compare a short string instead |
| `identity_not_known` | this account has no signing identity for the code to carry | `bootstrapCrossSigning` |
...
| `private_keys_not_held` | verifying another user puts this account's own key in the code, and this device cannot prove it holds one | `bootstrapCrossSigning`, or `recoverIdentity`. Verifying your own new login does not need them |
...
#### Five more things to design around
...
* **Scanning is not verifying.** When `submitScannedCode` resolves, nothing is verified: the other side has still to confirm and messages have to cross. What the flow produces at the end is the same `signature_upload` a string comparison produces, and the same last step everyone omits, which is fetching the other user's keys again afterwards.
* **No `trust_changed` arrives when a code flow finishes.** The producer reads the short-string comparison's own final state, and a code flow never has one, so a product that waits on the signal after `confirmScan` waits forever. The device does move: read `getDeviceStatuses`, which is what that signal tells you to do anyway. Named as a gap rather than a decision, and it is being closed alongside the stage.
* **How far a code flow has got is the one thing you cannot read yet.** `getVerificationStage` describes every flow in the vocabulary of a string comparison, so a flow that moved to a code reports what a string flow would, and `confirmScan` answers `wrong_stage` both when nobody has scanned yet and when the flow is over. Those two want opposite things done about them. A stage that can name a scanned flow's own states is being built beside this and is not in what you are reading, so take `getVerificationStage`'s own doc comment, rather than this paragraph, as the account of the copy you hold.
...
| Device verification by short string comparison (SAS) | working, in both flow shapes and whichever side opens the comparison, proven against a bare `matrix-sdk-crypto` machine driven directly: an agreement completing, and a genuine disagreement refusing. One of two methods since this release, and no longer the whole of verification |
| Device verification by a scannable code | working, and **off until a product calls `offerScannableCodes`**. All three modes the protocol defines: another user, your own new login with the established device showing, and your own new login with the new login showing. Proven against a bare `matrix-sdk-crypto` machine driven directly, in each mode, with this library showing the code and that machine reading it, and once in the other direction: a code that machine drew, read back through this library, confirmed, and both sides finishing. The library sees no camera and draws no picture: it hands over the bytes and the grid of squares, and the product owns the scanner and the screen. A build that never asks for codes announces on the wire what every earlier release announced, which is a test rather than a claim. Not yet read by a foreign scanner, and not yet reported through `getVerificationStage`, both of which the roadmap covers |
...
M2, the encryption core, M3, device verification by short string, M4, cross-signing and recovery, and M5, verification by a scannable code, have landed; the table above is what they produced. This sentence named M2 and M3 alone for two milestones after that stopped being the whole list, while the table under it grew rows neither of them produced. Two things are worth recording about the first two. A tokio runtime became mandatory, because group key sharing reaches `tokio::task::spawn`; the core owns one, and signal delivery is non blocking, so no callback holds a lock or waits on JavaScript. And binary size went the other way from expected: linking the Rust as a shared library instead of a static archive cut the published tarball by 74 percent, from 263 MB to 68 MB, which is 44 percent of its budget, so splitting into per platform packages was not needed.
...
**That size figure is a measurement of two architectures and a projection to the other four.** Scaled by architecture count it comes to roughly 299 KB on the 66,226 KB tarball `release-v0.1.1` packed, which moves the artifact from 43.11 percent of its 150 MB gate to about 43.31 percent, and consumes about a third of one percent of the 87 MB of headroom. Read that as an order of magnitude rather than as a number: the per-architecture delta could differ by tens of kilobytes on `armeabi-v7a` or `x86`, and it could not plausibly differ by the two orders of magnitude that would matter to this gate. `artifact-sizes.json` still carries no row for it, and will not until a release build records one, for the same reason it carries no M3 row: measuring one needs both platform legs built from this tree, which is the release workflow's job and not something a developer's machine does incidentally. A number taken from whatever binary happened to be lying around would be a real measurement of the wrong artifact and would read exactly like a real measurement of the right one, so `scripts/measure-artifacts.sh` refuses that case rather than recording it.

Three things about codes are not done. **No foreign scanner has read one**, which is the claim that matters most for a product whose users will point whatever camera their phone shipped with at a square, and which no in-process test can make. **`getVerificationStage` cannot yet describe a code flow's own states**, so `confirmScan` cannot tell a caller whether to wait or to start again. And **a flow that finishes by a scan announces no `trust_changed`**, because the producer reads the comparison's own state and a code flow has no comparison; the device does move, and `getDeviceStatuses` says so. The last two are the same piece of work, the stage vocabulary growing to carry a scanned flow, and it is not in what you are reading.
...
* a scannable code read by an ordinary phone camera, and a code flow's own stages readable through `getVerificationStage`. The method itself has landed and the two paragraphs above say what it cost and what it still owes; this line used to say the method was next
