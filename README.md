...
**This used to be described here as a race between that instant and your call, and that wording was wrong in a way that mattered.** It reads as though acting quickly were enough. It was not: the precondition was **as old as the last answer**, and nothing shortened it. `accountKeysFetched` never goes back to `false` once an answer has landed, the layer underneath volunteers no key query for an account it already tracks, and the `'identity_not_known'` refusal that sends you to this call queues nothing. So a product that had drained its pump to empty and reported every answer honestly, which is the careful thing to do, had **no** way left to refresh the one fact this call reads. Measured on continuwuity and on Synapse, in exactly that state, it replaced another device's live identity; a less thorough product survived the same sequence only because an unsent key query happened to still be sitting in its pump. Being careful made it worse.

**So `createCrossSigningIdentity` now asks for itself before it publishes.** The first call refuses `'account_keys_stale'` with the key query already queued: drain, send, report, call again. **That round is not a formality.** It is where an identity another of your devices published comes back, and where this call refuses `'identity_already_exists'` instead of overwriting it. What remains is your own round trip between reporting the answer and calling again, which is in your hands and shrinks the promptlier you retry.
...
**If the publication does not land, nothing is lost, but finishing it is a decision and not a retry.** `createCrossSigningIdentity` writes the identity to the store and then hands you the upload. Between those two moments the store holds an identity your account does not have, and a killed process, an offline device or a timed-out request leaves exactly that on disk. `getIdentityStatus().identityPublicationPending` is `true` while that is the case, and **finishing it is `createCrossSigningIdentity` again**, deliberately, on the same grounds you decided the first time, and after the same fresh-answer round as any other creation. On a relaunch that costs one `'account_keys_stale'` refusal and one more turn of the loop you already run, and then you are handed the identical publication.

**Read `identityPublicationPending` knowing that it says two things in one word.** `true` means *finish your own publication*, and it equally means *you are about to overwrite the identity your other phone made while this one was offline*. Nothing you can read from this device tells them apart. What tells them apart is the fresh-answer round above, and it happens before anything is published: if the answer carries an identity another of your devices made, `identityPublicationPending` goes `false` on its own, `privateKeysHeld` goes `false` with it, and the next `createCrossSigningIdentity` refuses `'identity_already_exists'`. If it carries nothing, the flag stays `true` and the publication proceeds. So the flag is still ambiguous when you read it; the ambiguity is now resolved before the damage rather than by it.
