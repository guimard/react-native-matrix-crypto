...
describe('cameraProofChecks', () => {
  it('promises exactly the pinned five steps, in order', () => {
    ...
    const checks = cameraProofChecks(
      fold([WAITING, FLOW_ANNOUNCED, CODE_ON_SCREEN, SCANNED, DONE]),
    )
  ...
  it('reports all five PASS on the happy path', () => {
    const checks = cameraProofChecks(
      fold([STARTING, WAITING, FLOW_ANNOUNCED, CODE_ON_SCREEN, SCANNED, DONE]),
    )
    expect(checks.every(check => check.ok)).toBe(true)
    expect(checks.filter(check => check.ok).length).toBe(
      CAMERA_PROOF_STEPS.length,
    )
  })
