...
export const FLOW_STEPS: FlowStep[] = [
  ...
  },
  {
    ...
    why: "Three functions in this library's product surface are final, compiling types that reject. Two of the three are refused on purpose rather than pending: exportSecrets and importSecrets would need a container Matrix does not define, so the byte array would be a format this library invented and no other client could read, and createRecovery delivers the interoperable form instead. This card used to say the implementation was still to come and that they would start working the moment the native side landed, which is a promise the library has since retracted for these two. It has pointed at two other functions before them, encryptEvent and then getDeviceStatuses, and each time the library implemented what the card called missing and this card went on claiming it for a milestone. What has changed is not the wording, which had been corrected before and rotted anyway: a host-side test now calls this exact function on every CI run, so the next time it stops rejecting the build turns red before anyone sees this card turn red on a phone.",
  },
  {
    id: 'layers',
