...
One thing about codes is not done. **No foreign scanner has read one**, which is the claim that matters most for a product whose users will point whatever camera their phone shipped with at a square, and which no in-process test can make. This paragraph named three. The other two were the same piece of work, the stage vocabulary growing to carry a scanned flow, and it landed: `getVerificationStage` answers `code-scanned`, and a flow that finishes by a scan announces `verification_completed` rather than the `trust_changed` it produces no producer for.
...
* a scannable code read by an ordinary phone camera. The method itself has landed and the two paragraphs above say what it cost and what it still owes; this line used to say the method was next, and then that a code flow's own stages were still unreadable, which `getVerificationStage`'s `code-scanned` closed
