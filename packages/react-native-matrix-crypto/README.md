...
**Every build gate has been watched rejecting a real violation**, not merely passing. There are twelve, they all run in CI, and a gate nobody has watched fail is not known to work. This sentence said ten while the table below listed eleven and `package.json` declared eleven. `gate:readme` held the rows, the scripts and the workflow to each other and counted nothing, so nothing failed; it counts now, and reads this sentence to do it.
...
**That size figure is a measurement of two architectures and a projection to the other five.** Seven ship: three iOS targets and four Android ABIs. Scaled over all seven and compressed, since the gate is on the packed tarball, it comes to roughly 299 KB on the 66,226 KB tarball `release-v0.1.1` packed, which moves the artifact from 43.11 percent of its 150 MB gate to about 43.31 percent and consumes about a third of one percent of the 87 MB of headroom. Uncompressed the same scaling is about 929 KB against a 193,565 KB unpacked tree, which is the same 0.5 percent said the other way. Read either as an order of magnitude rather than as a number: the per-architecture delta could differ by tens of kilobytes on `armeabi-v7a` or `x86`, and it could not plausibly differ by the two orders of magnitude that would matter to this gate. `artifact-sizes.json` still carries no row for it, and will not until a release build records one, for the same reason it carries no M3 row: measuring one needs both platform legs built from this tree, which is the release workflow's job and not something a developer's machine does incidentally. A number taken from whatever binary happened to be lying around would be a real measurement of the wrong artifact and would read exactly like a real measurement of the right one, so `scripts/measure-artifacts.sh` refuses that case rather than recording it.
