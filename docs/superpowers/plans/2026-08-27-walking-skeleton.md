...
export async function runProbe(input: string, payload: Uint8Array): Promise<ProbeResult> {
  try {
    const report = await probeWithObserver(input, payload, {
      onSignal(signal: { kind: string; detail: string }) {
        emitCryptoSignal({ kind: 'probe_started', detail: signal.detail })
      },
    })
    return { echoed: report.echoed, payload: report.payload, coreVersion: report.coreVersion }
  } catch (e) {
...
TS_HITS=$(grep -rnE '\bconsole\.[a-z]+' \
  packages/react-native-matrix-crypto/src \
